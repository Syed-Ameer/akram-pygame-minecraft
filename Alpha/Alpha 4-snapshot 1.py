...
USE_EXPERIMENTAL_TEXTURES = True  # Toggle for block textures
...
class Weather:
        ...
        if self.weather_timer >= self.weather_duration:
            if random.random() < 0.05:  # 5% chance to start weather
                if biome_type == SNOW_BIOME:
                    ...
                    self.is_raining = False
                elif biome_type in [OAK_FOREST_BIOME, JUNGLE_BIOME, TAIGA_BIOME]:
    ...
    def draw(self, surface, camera_x, camera_y):
        """Draw rain/snow particles accounting for camera position"""
        if self.is_raining:
            for particle in self.rain_particles:
                # Draw rain relative to screen, not world (rain is screen overlay)
                pygame.draw.line(surface, (150, 180, 255), 
                               (particle['x'], particle['y']),
                               (particle['x'] - 2, particle['y'] + 15), 2)
...
BLOCK_TYPES = {
    ...
    315: {"name": "Bird Egg", "color": (100, 150, 200), "mineable": False, "solid": False, "spawn_egg": "Bird"},
...
try:
    ...
    texture_mapping = {
        ...
        7: r"..\Textures\golden_sword.png",  # Lava (joke texture)
        ...
        239: r"..\Textures\ender_pearl.png",  # Ender Pearl
        240: r"..\Textures\ender_eye.png",  # Ender Eye
...
class EnderPearl(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        row = self.rect.centery // BLOCK_SIZE
        if 0 <= row < GRID_HEIGHT and 0 <= col < GRID_WIDTH:
            if WORLD_MAP[row][col] != 0:  # Hit a block
                # Teleport player
                self.owner.rect.x = self.rect.x - self.owner.rect.width // 2
                self.owner.rect.y = self.rect.y - self.owner.rect.height // 2
                print(f"✨ Teleported!")
                self.kill()
                return
...
class EyeOfEnder(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, player_x):
        ...
        self.lifetime = 180  # 3 seconds before dropping
        ...
        if distance > 0:
            speed = 8
            self.vel_x = (dx / distance) * speed
            self.vel_y = (dy / distance) * speed - 3  # Arc upward
        else:
            self.vel_x = 0
            self.vel_y = -5
...
while running:
    ...
    if CURRENT_MENU_STATE == MENU_STATE_USERNAME:
        ...
        current_biome = OAK_FOREST_BIOME
        if player_col < len(BIOME_MAP):
            current_biome = BIOME_MAP[player_col]
        ...
        for pearl in ENDER_PEARLS:
            # Draw ender pearl as purple circle
            pygame.draw.circle(screen, (128, 0, 128), 
                             (pearl.rect.centerx - camera_x, pearl.rect.centery - camera_y), 4)
        ...
        if WEATHER:
            camera_x, camera_y = calculate_camera_offset(player.rect)
            WEATHER.draw(screen, camera_x, camera_y)
