...
def add_cacti(world, height_map, start_col, end_col):
    ...
    for col in range(start_col, end_col):
        # 8% chance to spawn a cactus on sand
        if random.random() < 0.08:
            ground_row = height_map[col]
            
            # Ensure the block is Sand (ID 19)
            if ground_row < GRID_HEIGHT and world[ground_row][col] == 19:
...
def add_dead_bushes(world, height_map, start_col, end_col):
    ...
    for col in range(start_col, end_col):
        # 5% chance to spawn a dead bush
        if random.random() < 0.05:
            ground_row = height_map[col]
            
            # Ensure the block is Sand (ID 19)
            if ground_row < GRID_HEIGHT and ground_row > 0 and world[ground_row][col] == 19:
                # Place dead bush one block above sand
                if world[ground_row - 1][col] == 0:
                    world[ground_row - 1][col] = 22
...
def generate_world():
    ...
    # Create distinct biome zones across the world
    biome_map = []  # 0 = grassland, 1 = desert
    biome_size = 50  # Each biome is roughly 50 blocks wide
    
    for col in range(GRID_WIDTH):
        # Use a biome pattern that creates larger regions
        biome_phase = (col // biome_size) % 3
        if biome_phase == 0:
            biome_map.append(0)  # Grassland
        elif biome_phase == 1:
    ...
    # --- Add Biome-Specific Decorations AFTER lakes are carved ---
    # Scan through biomes and add appropriate decorations
    ...
    for col in range(1, GRID_WIDTH + 1):
        ...
        if biome_changed:
            # Add decorations for the biome region we just finished
            if current_biome_type == 0:  # Grassland
                add_trees(WORLD_MAP, height_map)
            elif current_biome_type == 1:  # Desert
                add_cacti(WORLD_MAP, height_map, current_biome_start, col)
                add_dead_bushes(WORLD_MAP, height_map, current_biome_start, col)
    ...
    for col in range(GRID_WIDTH):
        ...
        surface_block = WORLD_MAP[ground_row][col] if ground_row < GRID_HEIGHT else 0
        
        # Grass (1) for grassland or Sand (19) for desert
        is_valid_surface = (surface_block == 1 or surface_block == 19)
        
        if ground_row < GRID_HEIGHT and is_valid_surface: 
            spawn_x = col * BLOCK_SIZE
            spawn_y = (ground_row - 2) * BLOCK_SIZE # Spawn two blocks above surface
            ...
            if zombies_spawned == 0 and abs(col - GRID_WIDTH // 2) < 5:
                ...
                # Random Mob Spawns - biome specific
                r = random.random()
                
                if is_desert:
                    # Desert-specific mobs
                    if r < 0.03:  # 3% chance for camel
                        mobs.add(Camel(spawn_x, spawn_y + BLOCK_SIZE))
                    # Hostile mobs spawn less in desert
                    elif r < 0.03 + 0.01:  # Zombie (rare)
                        mobs.add(Zombie(spawn_x, spawn_y))
                    elif r < 0.03 + 0.02:  # Spider
                        mobs.add(Spider(spawn_x, spawn_y))
                else:
                    # Grassland mobs (original spawning)
                    if r < 0.02: 
...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.mount_offset_y = -BLOCK_SIZE * 0.8  # Player sits on top of camel
...
class Camel(Mob):
    ...
    def __init__(self, x, y):
        super().__init__(x, y, BLOCK_SIZE * 1.2, BLOCK_SIZE * 1.5, (193, 154, 107)) # Tan/sandy color
        self.health = 15
        self.max_health = 15
        self.speed = 1.8  # Slightly faster than cows
        self.drop_id = 14  # Leather
        ...
        pygame.draw.rect(self.image, leg_color, (5, h - 15, 8, 15))  # Front left leg
        pygame.draw.rect(self.image, leg_color, (w - 13, h - 15, 8, 15))  # Front right leg
        pygame.draw.rect(self.image, leg_color, (15, h - 15, 8, 15))  # Back left leg
        pygame.draw.rect(self.image, leg_color, (w - 23, h - 15, 8, 15))  # Back right leg
        
        # Body (main rectangular body)
        body_color = (193, 154, 107)
        pygame.draw.rect(self.image, body_color, (5, h - 35, w - 10, 20))
        
        # Hump (blocky rectangles stacked)
        hump_color = (180, 140, 90)
        pygame.draw.rect(self.image, hump_color, (w // 2 - 8, h - 45, 16, 10))
        pygame.draw.rect(self.image, hump_color, (w // 2 - 5, h - 50, 10, 5))
        
        # Neck (vertical rectangle)
        pygame.draw.rect(self.image, body_color, (w - 15, h - 50, 10, 15))
        
        # Head (small rectangle)
        pygame.draw.rect(self.image, body_color, (w - 15, h - 55, 10, 8))
