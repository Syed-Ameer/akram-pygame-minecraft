...
def generate_world():
    ...
    lake_bottom_row = 0
    lake_started = False  # FIXED: Track if we just started a lake
    ...
    for col in range(GRID_WIDTH):
        ...
        if current_lake_width == 0:
            if random.random() < LAKE_PROBABILITY:
                ...
                lake_started = True  # FIXED: Mark that we just started a lake
        ...
        if current_lake_width > 0:
            ...
            # FIXED: Spawn narwhal when we just started a lake
            if lake_started: 
                ...
                lake_started = False  # FIXED: Reset the flag
...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.fall_velocity_threshold = 9.8  # Takes about 3 blocks of falling (reasonable with terminal velocity of 10)
        self.max_fall_vel = 0 
...
class Narwhal(Mob):
    ...
    def ai_move(self, player):
        ...
        if not in_water:
            # If not in water, try to sink down to find water
            self.vel_y = 2
        else:
    ...
    def update(self, player):
        ...
        # Don't call super().update() because it would apply gravity
        # Instead just handle collision manually
